...
// NOTE: PII/vision redaction intentionally does NOT happen here.
// `getState()` is called fresh, independently, both when building a prompt
// for the LLM (network-bound) and when resolving an element right before a
// local click/type action (never leaves the device) — each call returns a
// brand-new DOMElementNode tree from a fresh extraction. Sanitizing
// unconditionally in this file would only add cost with no benefit to the
// action-execution callers, and (see Part 6) wouldn't give us anywhere to
// implement a fail-closed abort. Redaction instead happens exactly once, at
// the actual network boundary, in `background/agent/prompts/base.ts` via
// `privacy-engine/adapters/nanobrowser/NanoBrowserAdapter`, which mutates
// that specific tree in place before NanoBrowser's own
// `clickableElementsToString()` serializes it for the model. The two
// functions previously imported here (`sanitizeDOMState`/`sanitizeScreenshot`
// from the old `@src/redaction/*` module) were dead code — this file only
