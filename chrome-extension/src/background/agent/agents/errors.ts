...
/**
 * Thrown when the privacy engine cannot guarantee that page context is safe
 * to send to the remote LLM (fail-closed). This is treated the same as an
 * auth/config error: it aborts the current step/task immediately rather than
 * being retried, since retrying with the same unanalyzable page would just
 * fail again — and silently downgrading to "send raw data anyway" would
 * defeat the entire privacy guarantee.
 */
export class PrivacyBlockedError extends Error {
  constructor(
    ...
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, PrivacyBlockedError);
    }
  }
